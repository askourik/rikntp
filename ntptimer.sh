...
#logger "ntptimer.sh method=$method"
arr1=(${method// / })
mode1=${arr1[1]}
mode2=$(echo $mode1 | tr -d \") 
arr2=(${mode2//./ })
mode=${arr2[5]}
logger "ntptimer.sh mode=$mode"
...
# fi
 ...
 logger "ntptimer.sh numserv=$numserv"
 ...
 for i in "${!arr3[@]}"; do 
  ...
  if [ $i -gt 1 ];  then
    ...
    then
      sleep 5
      logger "ntptimer.sh modenew=$mode ntpservernew=$ntpserver"
      newdate=$(date) 
      logger "ntptimer.sh set NTP from $ntpserver = $newdate"
      #logger "ntptimer.sh NTP write $mode $ntpserver to rikntp.conf"
      #echo $mode $ntpserver > /etc/rikntp/rikntp.conf
      sleep 1
      break
    fi
    logger "ntptimer.sh set NTP from $ntpserver failed, trying next"
    sleep 5
  fi
 done
